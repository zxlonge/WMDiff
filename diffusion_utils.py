...
# Forward functions
def q_sample(y, y_0_hat, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t, noise=None, mapped_t=None,
             SingleGranularities=False):
    """
    y_0_hat: prediction of pre-trained guidance classifier; can be extended to represent 
        any prior mean setting at timestep T.
    """
    if not SingleGranularities:
        if noise is None:
        ...
        # 使用 torch.gather 函数选择 y_0_hat 的子张量
        indices = mapped_t.long()
        selected_y_0_hat = torch.zeros_like(y)
        for i in range(y_0_hat.shape[0]):

            selected_y_0_hat[i, :] = y_0_hat[i, indices[i], :]

        y_t = sqrt_alpha_bar_t * y + (1 - sqrt_alpha_bar_t) * selected_y_0_hat + sqrt_one_minus_alpha_bar_t * noise
